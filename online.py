...
def online_NN(training,label_training,weights,learning_rate):
  error=error_func(weights,training[0,:],label_training[0,:])
  converage=0.001  
  num_iteration=1
  ...
  gradient = log_gradient(weights, training[0,:], label_training[0,:])
  norm_g=np.linalg.norm(gradient,1)
  norm_gradient[num_iteration-1]=norm_g
  index=0

  while (norm_g>converage and num_iteration<100000):
    index=index+1
    if(index==training.shape[0]):
      index=0
    weights=weights-learning_rate*(gradient)
    error=error_func(weights,training[index,:],label_training[index,:])
    num_iteration+=1
    change_error[num_iteration-1]=error
    gradient = log_gradient(weights, training[index,:], label_training[index,:])
    norm_g=np.linalg.norm(gradient,1)
    norm_gradient[num_iteration-1]=norm_g
...
if __name__ == "__main__":
  ...
  print('---------------------------------------------')
  print ('Perform online training using gradient descent')
  print('---------------------------------------------')
  #initialize weights
  ...
  weights,num_iteration,change_error,norm_gradient=online_NN(training,label_training,weights,learning_rate)
